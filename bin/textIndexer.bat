...
@echo off
if not exist "%NATAM_HOME%" goto NEED_HOME

:: Make a classpath containing all the jars in XTF_HOME\WEB-INF\lib
setlocal enabledelayedexpansion
set XTFCP=%CLASSPATH%
for %%i in ("%NATAM_HOME%"\WEB-INF\lib\*.jar) do set XTFCP=!XTFCP!;%%i

:: And fire off the command.
java -cp "%XTFCP%" -Xms50m -Xmx1000m -Dxtf.home="%NATAM_HOME%" -DentityExpansionLimit=128000 -enableassertions org.cdlib.xtf.textIndexer.TextIndexer %*
set XTFCP=
exit /B 0
                                          
:: Print error message and exit
:NEED_HOME
echo ERROR: NATAM_HOME environment variable not set properly.
